export const personal = {
  name: "Sri Hari Sirisipalli",
  title: "Product Developer",
  subtitle: "Apps, Websites, Industry Software, AI, Automation. Fractional CTO for Engineering R&D.",
  roles: [
    "Product Developer",
    "Fractional CTO",
    "AI Systems Builder",
    "Automation Architect",
    "Researcher",
  ],
  bio: [
    "I do product development. Apps, websites, industry software, AI systems, and automation. Whatever the surface, I build it end-to-end.",
    "Fractional CTO at Openct. Independent practice hired when a project needs hard engineering depth and production delivery in one place.",
  ],
  stats: [
    { value: 3, suffix: "+", label: "Years Experience" },
    { value: 18, suffix: "+", label: "Products Shipped" },
    { value: 7, suffix: "+", label: "Companies" },
    { value: 5, suffix: "+", label: "Industry Domains" },
  ],
  email: "sriharisirisipalli0@gmail.com",
  location: "Visakhapatnam, India",
  calendlyUrl: "https://calendly.com/sriharisirisipalli0",
  formspreeEndpoint: "https://formspree.io/f/meqwqdal",
  socialLinks: [
    {
      platform: "LinkedIn",
      url: "https://www.linkedin.com/in/sri-hari-sirisipalli/",
      icon: "Linkedin",
    },
    {
      platform: "GitHub",
      url: "https://github.com/srihari-sirisipalli",
      icon: "Github",
    },
  ],
  resumeUrl: "/Sri_Hari_Sirisipalli_Resume.pdf",
};
